...
namespace ReturnTide.Workshop {
 public class WorkshopSpecimen:MonoBehaviour {
  ...
  public void Configure(WorkshopGame owner,int specimenNumber){
   game=owner;variant=specimenNumber%6==1?1:specimenNumber%6==3?2:specimenNumber%6==5?3:0;
   ...
   for(int i=0;i<organs.Length;i++)organs[i].containsCrystal=i==hidden;
  ...
  public void RevealLayer(TissueLayer layer){CurrentLayer=layer;foreach(var p in parts.Where(p=>p.layer==layer&&!p.Removed)){p.gameObject.SetActive(true);p.RefreshGuide(game.Tool==WorkshopTool.Scalpel);}if(layer==TissueLayer.Skin&&game.state.peeler)StartCoroutine(AutoPeel());}
  System.Collections.IEnumerator AutoPeel(){foreach(var p in parts.Where(p=>p.layer==TissueLayer.Skin).ToArray()){yield return new WaitForSeconds(.45f);p.Complete();}}
  public void PartRemoved(WorkshopPart part){
   switch(part.layer){case TissueLayer.Skin:game.stockSkin++;break;case TissueLayer.Flesh:game.stockMeat++;break;case TissueLayer.Bone:game.stockBone++;break;case TissueLayer.Organ:game.stockMutation++;if(part.containsCrystal){var core=parts.First(p=>p.layer==TissueLayer.Core);core.gameObject.SetActive(true);game.Chime(880,.22f);}break;case TissueLayer.Core:game.stockCores++;game.stockCoreValue+=game.balance.coreValue*(variant==3?5:variant==1?2:1);Finished=true;game.specimenComplete=true;game.Chime(1080,.3f);break;}
