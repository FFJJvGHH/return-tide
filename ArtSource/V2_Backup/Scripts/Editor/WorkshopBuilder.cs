...
namespace ReturnTide.Workshop.Editor {
 public static class WorkshopBuilder {
  ...
  public static void Build(){
   ...
   Room();Lights();BuildResearcher();BuildCamera();BuildSpecimen();BuildTools();BuildUI();
  ...
  static void Room(){
   ...
   var merchant=Model("Assets/ReturnTide/Models/Fishmonger.fbx",trade,new Vector3(.1f,0,1.25f));merchant.transform.rotation=Quaternion.Euler(0,180,0);merchant.transform.localScale=Vector3.one*1.15f;
  ...
  static void Lights(){
   ...
   Spot("台灯聚光",root,new Vector3(-1.4f,3.92f,1.3f),new Vector3(0,1,0),C("FFE5AE"),20,110);
  ...
  static void BuildResearcher(){
   ...
   var model=Model("Assets/ReturnTide/Models/Mio_Researcher.fbx",actor.transform,Vector3.zero);var anim=actor.AddComponent<WorkshopAnimator>();game.researcher=anim;anim.model=model.transform;
   ...
   anim.eyes=eyes.ToArray();anim.leftShoulder=Node("左肩",actor.transform,new Vector3(-.27f,1.42f,0)).transform;anim.rightShoulder=Node("右肩",actor.transform,new Vector3(.27f,1.42f,0)).transform;
   ...
   anim.leftHand=Shape("左手套",actor.transform,new Vector3(-.4f,1.2f,.7f),Vector3.one*.14f,"Teal").transform;anim.rightHand=Shape("右手套",actor.transform,new Vector3(.4f,1.2f,.7f),Vector3.one*.14f,"Teal").transform;
  ...
  static void BuildSpecimen(){
   var root=Node("标本 · 皮 / 肉 / 骨 / 器官 / 晶核",null,game.specimenAnchor.position);var model=Model(Root+"/Models/Workshop_Specimen.fbx",root.transform,Vector3.zero);
   var specimen=root.AddComponent<WorkshopSpecimen>();game.specimen=specimen;var parts=new List<WorkshopPart>();
   ...
   foreach(var t in model.GetComponentsInChildren<Transform>().ToArray()){
    ...
    if(layer==1||layer==2){var points=new List<Transform>();for(int j=0;j<5;j++){var point=Node("切线点 "+j,wrapper.transform,new Vector3(0,bounds.extents.y+.045f,Mathf.Lerp(-bounds.extents.z*.8f,bounds.extents.z*.8f,j/4f))).transform;points.Add(point);}part.cutPath=points.ToArray();
   ...
   game.specimenPrefab=PrefabUtility.SaveAsPrefabAsset(root,Root+"/Prefabs/DissectableFish.prefab").GetComponent<WorkshopSpecimen>();
  }
