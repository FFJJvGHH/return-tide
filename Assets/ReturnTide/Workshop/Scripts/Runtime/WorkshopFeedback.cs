...
namespace ReturnTide.Workshop {
 public class WorkshopFeedback:MonoBehaviour {
  ...
  public Material trophyMaterial;public Transform trophyRoot;public AudioSource audioSource;
  ...
  void Update(){
   ...
   CueVisible=work&&(!game.PointerBusy&&pulseWindow||dragged);CueGlyph=dragged?(dragged.toxic?TideGlyph.Warning:TideGlyph.Tray):WantedTool==-2?(game.GoodsValue>0?TideGlyph.Bell:TideGlyph.Fish):WantedTool<0?TideGlyph.Lock:(TideGlyph)WantedTool;
   CuePosition=dragged?(dragged.toxic?game.isolationTray:game.outputTray).position:WantedTool==-2?(game.GoodsValue>0?new Vector3(3.35f,2.2f,2.4f):new Vector3(-3.65f,1.8f,-1.4f)):WantedTool<0?game.specimen.transform.position+Vector3.up:toolSignals[Mathf.Clamp(WantedTool,0,3)].transform.position+Vector3.up*.55f;
  ...
  IEnumerator Receipt(TissueLayer layer,bool toxic,int yieldCount,float delay){yield return new WaitForSeconds(delay);LandingEvents++;Transform tray=toxic?game.isolationTray:game.outputTray;Play(settings.ceramic,.35f,layer==TissueLayer.Core?1.3f:1);if(toxic)leftPulse=Time.time+.65f;else rightPulse=Time.time+.65f;Emit(tray.position+Vector3.up*.2f,toxic?settings.gold:settings.mint,6,.55f,.025f);
   ...
   if(layer!=TissueLayer.Shell){overlay.Gain(glyph,layer==TissueLayer.Flesh?yieldCount:1,tray.position+Vector3.up*.3f);DepositToken(layer,toxic);}if(layer==TissueLayer.Core){Play(settings.core,.45f);game.researcher.Nod();}
  }
  void DepositToken(TissueLayer layer,bool toxic){if(trophies.Count>=9){Destroy(trophies[0]);trophies.RemoveAt(0);}var go=Instantiate(sparkPrefab,trophyRoot);go.name="盘中收获 · "+layer;go.GetComponent<Renderer>().sharedMaterial=trophyMaterial;go.SetActive(true);Transform tray=toxic?game.isolationTray:game.outputTray;int i=receiptSlot++;go.transform.position=tray.position+new Vector3(((i%3)-1)*.19f,.10f,((i/3)%3-1)*.26f);go.transform.localScale=layer==TissueLayer.Flesh?new Vector3(.26f,.065f,.18f):layer==TissueLayer.Core?new Vector3(.10f,.22f,.10f):new Vector3(.16f,.06f,.10f);Tint(go.GetComponent<Renderer>(),layer==TissueLayer.Flesh?new Color(.78f,.42f,.43f):layer==TissueLayer.Core?settings.mint:toxic?new Color(.42f,.65f,.3f):settings.gold);trophies.Add(go);}
  public void Sold(int amount,Vector3 from,bool clear=true){Activity();Play(settings.coin,.5f);overlay.Gain(TideGlyph.Coin,amount,from);Emit(from,settings.gold,16,1.1f,.035f);if(clear){foreach(var go in trophies.Where(t=>t&&!t.name.EndsWith("Organ")).ToArray()){Destroy(go);trophies.Remove(go);}}}
