...
namespace ReturnTide.Workshop {
 public class WorkshopGame:MonoBehaviour {
  ...
  public void Begin(){if(finishedCampaign){SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex);return;}if(Playing)return;Playing=true;specimen.ShowGuides(Tool);hud.SetMode(true,false);cameraRig.SetView(WorkshopView.Bench);}
  void Update(){
  ...
  public void ProcessPointer(Vector2 position,bool down,bool held,bool up){
   ...
   if(up){
    ...
    if(dragging){Deposit(dragging,cursorPoint);dragging=null;researcher.Grip(false);}
   }
  ...
  public void SetTrade(bool value){CancelPointer();hud.intakeGroup.SetActive(false);Trading=value;hud.SetMode(true,value);cameraRig.SetView(value?WorkshopView.Trade:WorkshopView.Bench);laserBeam.enabled=false;}
  ...
  public bool SellGoods(){if(GoodsValue<=0)return false;state.coins+=GoodsValue;state.deliveries++;state.everSold=true;stockSkin=stockMeat=stockBone=stockCores=stockCoreValue=0;state.corePrices.Clear();state.coreQualities.Clear();researcher.Nod();Chime(680,.2f);Save();return true;}
  public void DamagePurity(int amount,string feedback){if(restoring||specimenComplete)return;state.purity=Mathf.Max(0,state.purity-amount);hud.Toast(feedback);Save();}
